        ...
        <div className="grid gap-4 py-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <p><span className="font-medium">Market Cap:</span> {formatCurrency(stock.marketCap)}</p>
            </div>
            <div>
              <h3 className="font-semibold mb-2">Company Overview</h3>
              <p><span className="font-medium">CEO:</span> {stock.ceo}</p>
              <p><span className="font-medium">Founded:</span> {stock.founded}</p>
              <p><span className="font-medium">Headquarters:</span> {stock.headquarters}</p>
              <p><span className="font-medium">Employees:</span> {stock.employees.toLocaleString()}</p>
            </div>
          </div>
          <Separator />
          <div>
            <h3 className="font-semibold mb-2">Financial Information</h3>
            <p><span className="font-medium">Revenue:</span> {formatCurrency(stock.revenue)}</p>
            <p><span className="font-medium">Net Income:</span> {formatCurrency(stock.netIncome)}</p>
          </div>
          <Separator />
          <div>
          ...
          <div>
            ...
            <p><span className="font-medium">Industry:</span> {stock.industry}</p>
            <p><span className="font-medium">Website:</span> <a href={stock.website} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">{stock.website}</a></p>
          </div>
        </div>
        <DialogFooter>
          <Button onClick={onClose}>Close</Button>
          <Button variant="default">Buy Stock</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  )
